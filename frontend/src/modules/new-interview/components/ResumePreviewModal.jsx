  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="lg">
      <div className="resume-preview-modal-content" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid var(--border-subtle, #E2E8F0)', paddingBottom: 16 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <div style={{ width: 44, height: 44, borderRadius: 10, background: 'rgba(15, 118, 110, 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#0F766E' }}>
              <FileText size={24} />
            </div>
            <div>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 4, fontSize: 12, color: 'var(--text-secondary, #64748B)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, background: '#F1F5F9', padding: 4, borderRadius: 8 }}>
          <button
            ...
            style={{
              ...
              color: activeTab === 'doc' ? '#fff' : 'var(--text-secondary)',
              fontSize: 13,
              fontWeight: 500,
              cursor: 'pointer',
              display: 'flex',
        ...
        {/* Body Viewer */}
        <div style={{ height: '420px', borderRadius: 8, overflow: 'hidden', background: '#F8FAFC', border: '1px solid var(--border-subtle, #E2E8F0)' }}>
          {activeTab === 'doc' ? (
            isPdf || isImage ? (
              <iframe
                src={fileUrl}
              ...
              <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, padding: 24, textAlign: 'center' }}>
                ...
                <div>
                  <h4 style={{ margin: 0, fontSize: 16, color: 'var(--text-primary, #0F172A)' }}>Document Ready for Processing</h4>
                  <p style={{ margin: '6px 0 0', color: 'var(--text-secondary, #64748B)', fontSize: 13 }}>
            ...
            <div style={{ height: '100%', padding: 16, overflowY: 'auto', color: '#334155', fontSize: 13, lineHeight: '1.6', whiteSpace: 'pre-wrap', fontFamily: 'monospace' }}>
              {resume.textSnippet && resume.textSnippet !== 'Extracted text processing...'
                ? resume.textSnippet
                : 'Extracted text analysis will appear here during interview question synthesis.'}
            </div>
          )}
        </div>

        {/* Footer Actions */}
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, paddingTop: 12, borderTop: '1px solid var(--border-subtle, #E2E8F0)' }}>
